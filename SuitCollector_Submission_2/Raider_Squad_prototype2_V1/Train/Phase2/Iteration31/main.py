...
def main():
    ...
    for i in range(1):
        ...
        #getModelJson('./model_726600_20230310103605.h5',(16,),[1000],42)
...
def playwithAssistedAgent(env, total):
    for i in range(total):
        ...
        while done != True:
            ...
            p1 = int(y[0])
            p2 = int(y[1]) 
            x = env.getActionFromCoOrds(p1,p2)
            print("action is ",x)
...
def getModelJson(saved_weights_location, fl, ml, ll):
    # input layer
    inputs = layers.Input(shape=fl)

    # Hidden layers
    hiddenLayers = []
    for i in range(len(ml)):
        if i==0:
            hiddenLayers.append(layers.Dense(ml[i], activation="relu")(inputs))
        else:
            hiddenLayers.append(layers.Dense(ml[i], activation="relu")(hiddenLayers[i-1]))
            
    #layer1 = layers.Dense(4096, activation="relu")(inputs)
    #layer2 = layers.Dense(45, activation="relu")(layer1)
    #layer3 = layers.Dense(45, activation="relu")(layer2)
    #layer4 = layers.Dense(45, activation="relu")(layer3)   
    #layer5 = layers.Dense(10, activation="relu")(layer4)

    # output layer    
    action = layers.Dense(ll, activation="linear")(hiddenLayers[-1])
